...
<div class="container">
@if(Session::has('message_error_desassociar'))
	<p class="alert {{ Session::get('alert-class', 'alert-info') }}">{{ Session::get('message_error_desassociar') }} <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a></p>
@endif
@if(Session::has('message_success_desassociar'))
	<p class="alert {{ Session::get('alert-class', 'alert-info') }}">{{ Session::get('message_success_desassociar') }} <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a></p>
@endif
	</center>
			...
			<div class="panel-heading">		
				<div class="form-group">
					<div class="col-sm-offset-0 col-sm-0">						    
						<a href ="{{ route('indicador.desassociar') }}"> <button type="button" id="btnvoltar" class="btn btn-success">
							<i class="fa fa-chevron-left"> Voltar</i> 
						</button></a> <center><b><h3>Selecionar Indicador a ser Removido do Projeto</h3></b></center>
					</div>
				</div>
			</div>
				...
				<div class="panel-body">
				  <div class="alert-warning">
				  	<span class="glyphicon glyphicon-alert"> 
		                <b> Caso o indicador possua acompanhamento e valor informado, estes ser&atilde;o removidos definitivamennte&#33;</b></span>
				  </div>
				  <div class="">		
								...
								@endphp												
									@foreach($indicadores as $indic)
										...
										<tr><td>
											...
											<td>
											   <form id="formIndic-{{$indic->id}}"action="{{ url('indicadore/remover/'.$indic->id )}}" method="post" clsss="float:left">
												  {!! csrf_field() !!} 
												  {{ method_field('PUT') }}
													 ...
													 <input type="hidden" name="faseprojeto" id="indicador" value="{{$indic->id}}" class="form-control">
													 <input type="hidden" name="indicador" id="indicador" value="{{$indic->indicador_id}}" class="form-control">
													 <input type="hidden" name="user" id="user" value="{{Auth::user()->id}}" class="form-control">
												
													<a href="{{ url('indicadore/remover/'.$indic->id)}}">
													  <button  title="@php echo utf8_encode('Desassociar Indicador do Projeto');@endphp" type="submite" class="btn btn-sm btn-danger" id="'btn-{{$projeto->id}}'">
			...
			<script type="text/javascript">
						$(document).ready(function () {
							...
							$('.btn-danger').click(function (event) {
								...
								var button = confirm("Quer desassociar este indicador do projeto?");	// return true or false								
								if(button == true)
								{
