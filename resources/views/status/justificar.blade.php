				...
				<div class="panel panel-heading">
					...
					<div class="form-group">
						<div class="col-sm-offset-0 col-sm-0">						    
							<a href ="{{ route('status.show') }}"> <button type="button" class="btn btn-success">
                ...
                <div class="panel-body">
					@if(!is_null($projeto))						
						...
						@for($contadorIndicador; $contadorIndicador < 9; $contadorIndicador++)
							...
							@php if($contadorIndicador == 5){ echo '<strong>'; echo utf8_encode('Atual Status do Projeto:'); echo'</strong>&nbsp;';echo '<b style="color:orange;">'. $projeto->getStatusDoProjeto($projeto->status_id).'</b>';} @endphp
							...
							<div class="form-group navbar-form">								
								<div id="div_jus_aprovada" style='display:none' class="formAcomp">									
									...
									<textarea rows="6" cols="50"  min="15" maxlength="255" value="" placeholder="@php echo utf8_encode('Inserir Texto da justificativa');@endphp" name="aprovada" 
											id="justificativaAprovada" class="acompanhar Aprovada requerido form-control" onKeyPress="return countChar(this); function countChar(val) { var len = val.value.length; if (len >= 255) {val.value = val.value.substring(0, 255);} else { $('#charNumAprov').text(255 - len);}}; " ></textarea>
								</div>
							</div>
							<!-- Add Task JUSTIFY -->
							<div class="form-group navbar-form">								
								<div id="div_jus_cancelada" style='display:none' class="formAcomp">		  									
									...
									<textarea rows="6" cols="50"  min="6" maxlength="255" value="" placeholder="@php echo utf8_encode('Inserir Texto da justificativa');@endphp" name="cancelada" 
											id="JustificativaCancelado" class="acompanhar requerido form-control" onKeyPress="return countChar(this); function countChar(val) { var len = val.value.length; if (len >= 255) {val.value = val.value.substring(0, 255);} else { $('#charNumCancelado').text(255 - len);}}; " ></textarea>
			...
			<script type="text/javascript">
						$(document).ready(function () {		
							...
							$('.btn-primary').click(function (event) {
								...
								var button = confirm("Quer alterar o status?");	// return true or false								
								if(button == true)
								{
